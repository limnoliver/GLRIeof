...
dat <- wq
out <- data.frame(matrix(nrow = length(trt_vars), ncol = 9))
...
for (i in 1:length(trt_vars)) {
  ...
  mean_pct_change_event <- mean(temp_after$pred_diff_perc)
  mean_pct_change_sum <- ((sum(temp_after$pred_treatment) - sum(temp_after$pred_baseline))/sum(temp_after$pred_baseline))*100
